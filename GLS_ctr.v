...
Inductive ctr_L (fml : MPropF V) : relationT (rel (list (MPropF V))) :=
  | ctr_LI Γ0 Γ1 Γ2 Δ : ctr_L fml
        (Γ0 ++ fml :: Γ1 ++ fml :: Γ2, Δ) (Γ0 ++ fml :: Γ1 ++ Γ2, Δ).
Inductive ctr_R (fml : MPropF V) : relationT (rel (list (MPropF V))) :=
...
- destruct s.
  ...
  + destruct p. destruct x.
    ...
    * inversion e0. subst. 
      assert (E: (Γ3 ++ ⊥ V :: x) ++ A :: Γ1 ++ A :: Γ2 = Γ3 ++ ⊥ V :: x ++ A :: Γ1 ++ A :: Γ2).
...
- destruct p. destruct x.
  ...
  + inversion e0. subst. apply app2_find_hole in H2. destruct H2. destruct s.
    * destruct s.
      { destruct p. subst.
        assert (E: Γ0 ++ m :: Γ1 ++ m :: ⊥ V :: Γ4 = (Γ0 ++ m :: Γ1 ++ [m]) ++ ⊥ V :: Γ4).
        repeat rewrite cons_single. repeat rewrite <- app_assoc. reflexivity. rewrite E.
        apply BotLRule_I. }
      { destruct p. subst.
        assert (E: Γ0 ++ m :: Γ1 ++ m :: x0 ++ ⊥ V :: Γ4 = (Γ0 ++ m :: Γ1 ++ m :: x0) ++ ⊥ V :: Γ4).
        repeat rewrite <- app_assoc. simpl. repeat rewrite <- app_assoc. reflexivity.
        rewrite E. apply BotLRule_I. }
    * destruct p. destruct x0.
      { simpl in e1. subst.
        assert (E: Γ0 ++ m :: (x ++ []) ++ m :: ⊥ V :: Γ4 = (Γ0 ++ m :: x ++ [] ++ [m]) ++ ⊥ V :: Γ4).
        repeat rewrite cons_single. repeat rewrite <- app_assoc. reflexivity.
        rewrite E. apply BotLRule_I. }
      { inversion e1. subst.
        assert (E: Γ0 ++ m :: (x ++ ⊥ V :: x0) ++ m :: Γ2 = (Γ0 ++ m :: x) ++ ⊥ V :: x0 ++ m :: Γ2).
...
(in case the contracted formula was weakened) or contracted versions of ps, they
are derivable/provable if ps are. *)
...
Theorem GLS_hpadm_ctr_LR : forall (k : nat) s
        ...
        k = (derrec_height D0) ->
          ...
          existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) sc),
          derrec_height D1 <= k)).
Proof.
...
(existsT2
  D1 : derrec (GLS_rules (V:=V))
         (fun _ : rel (list (MPropF V)) => False) sc,
  derrec_height D1 <= x)))).
...
{ inversion ctr. assert (DersNil: dersrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False) []).
  ...
  * inversion H1. rewrite <- H in H5. inversion H5.
    ...
    + destruct s.
      { destruct p. inversion e0. subst. pose (BotLRule_I Γ3 (Γ1 ++ Γ2) Δ). apply BotL in b.
        pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
        (ps:=[]) (Γ3 ++ ⊥ V :: Γ1 ++ Γ2, Δ) b DersNil). exists d0. simpl. repeat rewrite dersrec_height_nil.
        left. reflexivity. reflexivity. }
      { destruct p. destruct x.
        - rewrite app_nil_l in e0. inversion e0.
          subst. pose (BotLRule_I (Γ3 ++ []) (Γ1 ++ Γ2) Δ). apply BotL in b. simpl in b.
          pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
          (ps:=[]) ((Γ3 ++ []) ++ ⊥ V :: Γ1 ++ Γ2, Δ) b DersNil). exists d0. simpl.
          repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity.
        - inversion e0. subst.
          pose (BotLRule_I Γ3 (x ++ A :: Γ1 ++ Γ2) Δ). apply BotL in b. simpl in b.
          assert (E: Γ3 ++ ⊥ V :: x ++ A :: Γ1 ++ Γ2 = (Γ3 ++ ⊥ V :: x) ++ A :: Γ1 ++ Γ2). repeat rewrite <- app_assoc. reflexivity.
          rewrite E in b. clear E.
          pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
          (ps:=[]) ((Γ3 ++ ⊥ V :: x) ++ A :: Γ1 ++ Γ2, Δ) b DersNil). exists d0. simpl.
          repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity. }
    + destruct p. destruct x.
      { rewrite app_nil_l in e0. inversion e0. subst.
        pose (BotLRule_I Γ0 (Γ1 ++ Γ2) Δ). apply BotL in b. simpl in b.
        pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
        (ps:=[]) (Γ0 ++ ⊥ V :: Γ1 ++ Γ2, Δ) b DersNil). exists d0. simpl.
        repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity. }
      { inversion e0. subst. apply app2_find_hole in H9. destruct H9. destruct s.
        - destruct s.
          + destruct p. inversion e1. subst.
          ...
          + destruct p. destruct x0.
            * rewrite app_nil_l in e1. inversion e1. subst.
              ...
              pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
              (ps:=[]) (Γ0 ++ ⊥ V :: Γ1 ++ Γ4, Δ) b DersNil). exists d0. simpl.
              repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity.
            * inversion e1. subst.
              ...
              pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
              (ps:=[]) (Γ0 ++ m0 :: Γ1 ++ x0 ++ ⊥ V :: Γ4, Δ) b DersNil). exists d0. simpl.
              repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity.
        - destruct p. destruct x0.
          + rewrite app_nil_l in e1. inversion e1. subst.
            pose (BotLRule_I Γ0 ((x ++ []) ++ Γ2) Δ). apply BotL in b. simpl in b.
            pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
            (ps:=[]) (Γ0 ++ ⊥ V :: (x ++ []) ++ Γ2, Δ) b DersNil). exists d0. simpl.
            repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity.
          + inversion e1. subst.
            pose (BotLRule_I (Γ0 ++ m :: x) (x0 ++ Γ2) Δ). apply BotL in b. simpl in b.
            assert (E:  (Γ0 ++ m :: x) ++ ⊥ V :: x0 ++ Γ2 = Γ0 ++ m :: (x ++ ⊥ V :: x0) ++ Γ2). repeat rewrite <- app_assoc. reflexivity.
            rewrite E in b. clear E.
            pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
            (ps:=[]) (Γ0 ++ m :: (x ++ ⊥ V :: x0) ++ Γ2, Δ) b DersNil). exists d0. simpl.
            repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity. }
  (* ImpR *)
  ...
  * inversion X. rewrite <- H4 in X. pose (GLR_app_ctr_L ctr X). destruct s.
    { apply GLR in g0. pose (derI (rules:=GLS_rules (V:=V))
      (prems:=fun _ : rel (list (MPropF V)) => False) (ps:=[(XBoxed_list BΓ ++ [Box A0], [A0])]) 
...
{ inversion ctr. assert (DersNil: dersrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False) []).
  ...
  * inversion H1. rewrite <- H in H5. inversion H5.
    ...
    pose (derI (rules:=GLS_rules (V:=V))
    (prems:=fun _ : rel (list (MPropF V)) => False)
    (ps:=[]) (Γ0 ++ ⊥ V :: Γ1, Δ0 ++ A :: Δ1 ++ Δ2) b DersNil). subst. exists d0. simpl.
    repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity.
  (* ImpR *)
...
Theorem GLS_hpadm_list_ctr_L : forall Γ1 (k : nat) Γ0 Γ2 Γ3 Δ
        (D0 : derrec (@GLS_rules V) (fun _ => False) (Γ0 ++ Γ1 ++ Γ2 ++ Γ1 ++ Γ3,Δ)),
        k = (derrec_height D0) ->
          existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) (Γ0 ++ Γ1 ++ Γ2 ++ Γ3,Δ)),
          derrec_height D1 <= k.
Proof.
induction Γ1.
...
Theorem GLS_hpadm_list_ctr_R : forall Δ1 (k : nat) Γ Δ0 Δ2 Δ3
        (D0 : derrec (@GLS_rules V) (fun _ => False) (Γ,Δ0 ++ Δ1 ++ Δ2 ++ Δ1 ++ Δ3)),
        k = (derrec_height D0) ->
          existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) (Γ,Δ0 ++ Δ1 ++ Δ2 ++ Δ3)),
          derrec_height D1 <= k.
Proof.
induction Δ1.
- intros. exists D0. rewrite H. auto.
