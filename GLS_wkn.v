...
Inductive wkn_L (fml : MPropF V) : relationT (rel (list (MPropF V))) :=
  | wkn_LI Γ0 Γ1 Δ : wkn_L fml
        (Γ0 ++ Γ1, Δ) (Γ0 ++ fml :: Γ1, Δ).
Inductive wkn_R (fml : MPropF V) : relationT (rel (list (MPropF V))) :=
...
(in case the weakened formula is weakened in the rule) or weakened versions of ps, they
...
Theorem GLS_wkn_L : forall (k : nat) s
        ...
        k = (derrec_height D0) ->
          ...
          existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) sw),
          derrec_height D1 <= k)).
Proof.
...
(existsT2
  D1 : derrec (GLS_rules (V:=V))
         (fun _ : rel (list (MPropF V)) => False) sw,
  derrec_height D1 <= x)))).
...
- intros hei sw A wkn. inversion wkn. inversion g.
  ...
  * inversion H1. rewrite <- H in H5. inversion H5. apply app2_find_hole in H7.
    destruct H7. assert (DersNil: dersrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False) []).
    apply dersrec_nil.
    destruct s.
    + destruct s.
      { destruct p. inversion e0. subst. pose (IdPRule_I P (Γ2 ++ [A]) Γ3 Δ0 Δ1). apply IdP in i. rewrite <- app_assoc in i.
        simpl in i. pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
  ...
  * inversion H1. rewrite <- H in H5. inversion H5.
    ...
    + destruct s.
      { destruct p. inversion e0. subst.
        ...
        pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
        (ps:=[]) (Γ2 ++ A :: ⊥ V :: Γ3, Δ) b DersNil). exists d0. simpl. repeat rewrite dersrec_height_nil.
        left. reflexivity. reflexivity. }
      { destruct p. destruct x.
        - rewrite app_nil_l in e0. inversion e0. subst.
          pose (BotLRule_I ((Γ2 ++ []) ++ [A]) Γ3 Δ). apply BotL in b. rewrite <- app_assoc in b. simpl in b.
        ...
        - inversion e0. subst.
          ...
          assert (E0: Γ2 ++ ⊥ V :: x ++ A :: Γ1 = (Γ2 ++ ⊥ V :: x) ++ A :: Γ1). rewrite <- app_assoc. reflexivity.
          rewrite E0 in b.
          pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
          (ps:=[]) ((Γ2 ++ ⊥ V :: x) ++ A :: Γ1, Δ) b DersNil).
          exists d0. simpl. repeat rewrite dersrec_height_nil.
          left. reflexivity. reflexivity. }
    + destruct p. subst. pose (BotLRule_I (Γ0 ++ A :: x) Γ3 Δ). apply BotL in b.
      assert (E0 : (Γ0 ++ A :: x) ++ ⊥ V :: Γ3 = Γ0 ++ A :: x ++ ⊥ V :: Γ3).
      rewrite <- app_assoc. reflexivity. rewrite E0 in b.
      pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
      (ps:=[]) (Γ0 ++ A :: x ++ ⊥ V :: Γ3, Δ) b DersNil). exists d0. simpl.
  ...
  * inversion X. rewrite <- H4 in X. subst. pose (GLR_app_wkn_L wkn X). destruct s.
    { apply GLR in g0. subst. pose (derI (rules:=GLS_rules (V:=V))
...
- intros hei sw A wkn. inversion wkn. inversion g.
  ...
  * inversion H1. rewrite <- H in H5. inversion H5. apply app2_find_hole in H8.
    destruct H8. assert (DersNil: dersrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False) []).
    apply dersrec_nil.
    repeat destruct s.
  ...
  * inversion H1. rewrite <- H in H5. inversion H5.
    ...
    (ps:=[]) (Γ0 ++ ⊥ V :: Γ1, Δ0 ++ A :: Δ1) b DersNil). subst. exists d0. simpl.
    repeat rewrite dersrec_height_nil. left. reflexivity. reflexivity.
  (* ImpR *)
...
Theorem GLS_list_wkn_R : forall (k : nat) Γ Δ0 Δ1
        (D0 : derrec (@GLS_rules V) (fun _ => False) (Γ,Δ0 ++ Δ1)),
        k = (derrec_height D0) ->
          (forall l, existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) (Γ,Δ0 ++ l ++ Δ1)),
           derrec_height D1 <= k).
Proof.
intros. induction l.
- simpl. exists D0. lia.
...
Theorem GLS_list_wkn_L : forall (k : nat) Γ0 Γ1 Δ
        (D0 : derrec (@GLS_rules V) (fun _ => False) (Γ0 ++ Γ1,Δ)),
        k = (derrec_height D0) ->
          (forall l, existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) (Γ0 ++ l ++ Γ1,Δ)),
          derrec_height D1 <= k).
Proof.
intros. induction l.
- simpl. exists D0. lia.
...
Theorem GLS_XBoxed_list_wkn_L : forall (k : nat) Γ1 Γ0 Γ2 Δ
        (D0 : derrec (@GLS_rules V) (fun _ => False) (Γ0 ++ Γ1 ++ Γ2,Δ)),
        k = (derrec_height D0) ->
          (existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) (Γ0 ++ (XBoxed_list Γ1) ++ Γ2,Δ)),
           derrec_height D1 <= k).
Proof.
induction Γ1.
