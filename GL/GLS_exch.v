...
obtain exchange. Here is how we define the relation related to that notion: *)
...
Inductive list_exch_L : relationT (rel (list (MPropF V))) :=
  | list_exch_LI Γ0 Γ1 Γ2 Γ3 Γ4 Δ : list_exch_L
      (Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4, Δ) (Γ0 ++ Γ3 ++ Γ2 ++ Γ1 ++ Γ4, Δ).
Inductive list_exch_R : relationT (rel (list (MPropF V))) :=
...
destruct H.
- destruct s. destruct s. rewrite e.
  assert (E: (x ++ ⊥ V :: x0) ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4 =
  x ++ ⊥ V :: x0 ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4).
  pose (app_assoc x (⊥ V :: x0) (Γ1 ++ Γ2 ++ Γ3 ++ Γ4)). rewrite <- e0.
  auto. rewrite E. apply BotLRule_I.
- destruct s.
  + destruct s. destruct s. rewrite e.
    assert (E: Γ0 ++ Γ1 ++ Γ2 ++ (x ++ ⊥ V :: x0) ++ Γ4 =
    Γ0 ++ Γ1 ++ Γ2 ++ x ++ ⊥ V :: x0 ++ Γ4).
    pose (app_assoc x (⊥ V :: x0) Γ4).
    simpl in e0. rewrite <- e0. reflexivity. rewrite E.
    assert (E1: Γ0 ++ Γ1 ++ Γ2 ++ x ++ ⊥ V :: x0 ++ Γ4 =
    (Γ0 ++ Γ1 ++ Γ2 ++ x) ++ ⊥ V :: x0 ++ Γ4).
    pose (app_assoc (Γ0 ++ Γ1 ++ Γ2) x (⊥ V :: x0 ++ Γ4)).
    pose (app_assoc (Γ0 ++ Γ1) Γ2 x).
    pose (app_assoc Γ0 Γ1 Γ2). rewrite <- e2 in e1. rewrite <- e1 in e0.
    pose (app_assoc Γ0 Γ1 (Γ2 ++ x)). rewrite <- e3 in e0. rewrite <- e0.
    pose (app_assoc (Γ0 ++ Γ1) Γ2 (x ++ ⊥ V :: x0 ++ Γ4)). rewrite <- e2 in e4.
    rewrite <- e4.
    pose (app_assoc Γ0 Γ1 (Γ2 ++ x ++ ⊥ V :: x0 ++ Γ4)). rewrite e5. reflexivity.
    rewrite E1. apply BotLRule_I.
  + destruct s.
    * destruct s. destruct s. rewrite e.
      assert (E: Γ0 ++ Γ1 ++ (x ++ ⊥ V :: x0) ++ Γ3 ++ Γ4 =
      (Γ0 ++ Γ1 ++ x) ++ ⊥ V :: x0 ++ Γ3 ++ Γ4).
      pose (app_assoc x (⊥ V :: x0) (Γ3 ++ Γ4)). rewrite <- e0. simpl.
      pose (app_assoc (Γ0 ++ Γ1) x (⊥ V :: x0 ++ Γ3 ++ Γ4)).
      pose (app_assoc Γ0 Γ1 x). rewrite <- e2 in e1. rewrite <- e1.
      pose (app_assoc Γ0 Γ1 (x ++ ⊥ V :: x0 ++ Γ3 ++ Γ4)). rewrite <- e3.
      reflexivity. rewrite E. apply BotLRule_I.
    * destruct s.
      { destruct s. destruct s. rewrite e.
        assert (E: Γ0 ++ (x ++ ⊥ V :: x0) ++ Γ2 ++ Γ3 ++ Γ4 =
        (Γ0 ++ x) ++ ⊥ V :: x0 ++ Γ2 ++ Γ3 ++ Γ4).
        pose (app_assoc x (⊥ V :: x0) (Γ2 ++ Γ3 ++ Γ4)).
        rewrite <- e0. simpl.
        pose (app_assoc Γ0 x (⊥ V :: x0 ++ Γ2 ++ Γ3 ++ Γ4)).
        rewrite <- e1. reflexivity. rewrite E. apply BotLRule_I. }
      { destruct s. destruct s. rewrite e.
        assert (E: Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ x ++ ⊥ V :: x0 =
        (Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ x) ++ ⊥ V :: x0).
        pose (app_assoc (Γ0 ++ Γ1 ++ Γ2 ++ Γ3) x (⊥ V :: x0)).
        pose (app_assoc (Γ0 ++ Γ1 ++ Γ2) Γ3 x).
        pose (app_assoc (Γ0 ++ Γ1) Γ2 Γ3).
        pose (app_assoc Γ0 Γ1 Γ2).
        rewrite <- e3 in e2. rewrite <- e2 in e1.
...
apply list_exch_R_same_L with (Γ:=l) (Γe:=Γ0 ++ ⊥ V :: Γ1) (Δ:=l0) (Δe:=Δ) in exch.
...
of ps, they are derivable/provable if ps are. *)
...
Theorem GLS_adm_list_exch_L : forall s,
        ...
        (derrec (@GLS_rules V) (fun _ => False) se)).
Proof.
intros s D. apply derrec_all_rect with 
(Q:= fun x => forall (se : rel (list (MPropF V))),
list_exch_L x se ->
derrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False)
  se)
in D.
- exact D.
...
- intros ps concl rule. induction rule.
  ...
  * intros ders IH se exch. apply derI with (ps:=ps).
    + apply BotL. inversion b. symmetry in H0.
      assert (Permstose: existsT2 eΓ0 eΓ1, se = ((eΓ0 ++ Bot V :: eΓ1), Δ)).
...
- intros ps concl rule. induction rule.
  ...
  * intros ders IH se exch. apply derI with (ps:=ps).
    + apply BotL. inversion b. symmetry in H0.
      assert (Permstose: existsT2 eΔ, se = ((Γ0 ++ Bot V :: Γ1), eΔ)).
...
Inductive ExcClosure (hyps : rel (list (MPropF V)) -> Type) : rel (list (MPropF V)) -> Type :=
...
Theorem GLS_der_list_exch_L : forall hyps s,
        (derrec (@GLS_rules V) hyps s) ->
        (forall se, (@list_exch_L s se) ->
        (derrec (@GLS_rules V) (ExcClosure hyps) se)).
Proof.
intros hyps s D. apply derrec_all_rect with
(Q:= fun x => forall (se : rel (list (MPropF V))),
list_exch_L x se ->
derrec (GLS_rules (V:=V)) (ExcClosure hyps) se) in D.
- exact D.
...
Theorem GLS_hpadm_list_exch_R : forall (k : nat) s
                                  (D0: derrec (@GLS_rules V) (fun _ => False) s),
        k = (derrec_height D0) ->
        (forall se, (list_exch_R s se) ->
        (existsT2 (D1 : derrec (@GLS_rules V) (fun _ => False) se),
          derrec_height D1 <=k)).
Proof.
(* Setting up the strong induction on the height. *)
pose (strong_inductionT (fun (x:nat) => forall (s : rel (list (MPropF V)))
  (D0 : derrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False) s),
x = derrec_height D0 ->
(forall se,
(list_exch_R s se) ->
(existsT2
  D1 : derrec (GLS_rules (V:=V))
         (fun _ : rel (list (MPropF V)) => False) se,
  derrec_height D1 <= x)))).
...
- intros hei se exch. inversion exch.
  assert (DersNil: dersrec (GLS_rules (V:=V)) (fun _ : rel (list (MPropF V)) => False) []).
  ...
  * inversion H1. subst. inversion H5. subst. simpl.
  ...
  (* BotL *)
  * inversion H1. subst. inversion H5. subst. simpl.
    assert (BotLRule [] (Γ0 ++ ⊥ V :: Γ1, Δ0 ++ Δ3 ++ Δ2 ++ Δ1 ++ Δ4)). apply BotLRule_I. apply BotL in H.
    pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
    (ps:=[]) (Γ0 ++ ⊥ V :: Γ1, Δ0 ++ Δ3 ++ Δ2 ++ Δ1 ++ Δ4) H DersNil). exists d0. simpl. rewrite dersrec_height_nil.
    lia. reflexivity.
  (* ImpR *)
...
- intros hei se exch. inversion exch.
  ...
  * inversion H1. subst. inversion H5. subst. simpl.
  ...
  (* BotL *)
  * inversion H1. subst. inversion H5. subst. simpl.
    assert (In (⊥ V) (Γ0 ++ Γ3 ++ Γ2 ++ Γ1 ++ Γ4)). assert (In (⊥ V) (Γ0 ++ Γ1 ++ Γ2 ++ Γ3 ++ Γ4)).
    rewrite <- H0. apply in_or_app. right. apply in_eq. apply in_app_or in H. apply in_or_app.
    destruct H. auto. apply in_app_or in H. right. apply in_or_app. destruct H. right. apply in_or_app.
    right. apply in_or_app. auto. apply in_app_or in H. destruct H. right. apply in_or_app. auto.
    apply in_app_or in H. destruct H. auto. right. apply in_or_app. right. apply in_or_app. auto.
    apply in_splitT in H. destruct H. destruct s. rewrite e.
    assert (BotLRule [] (x ++ ⊥ V :: x0, Δ)). apply BotLRule_I. apply BotL in H.
    pose (derI (rules:=GLS_rules (V:=V)) (prems:=fun _ : rel (list (MPropF V)) => False)
    (ps:=[]) (x ++ ⊥ V :: x0, Δ) H DersNil). exists d0. simpl. rewrite dersrec_height_nil.
    lia. reflexivity.
  (* ImpR *)
  * inversion H1. subst. inversion H5. subst. simpl. simpl in IH.
