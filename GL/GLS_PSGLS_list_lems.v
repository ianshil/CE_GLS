...
Theorem in_splitT : forall x (l:list (MPropF V)), In x l -> existsT2 l1 l2, l = l1++x::l2.
Proof.
...
Lemma InT_dec : forall l (a : MPropF V) , (InT a l) + ((InT a l) -> False).
Proof.
induction l.
...
Lemma eq_dec_listsF : forall (l0 l1 : list (MPropF V)), {l0 = l1} + {l0 <> l1}.
Proof.
apply list_eq_dec. apply eq_dec_form.
Qed.

Lemma eq_dec_seqs : forall (s0 s1 : rel (list (MPropF V))), {s0 = s1} + {s0 <> s1}.
Proof.
...
Lemma seqs_in_splitT : forall (x : rel (list (MPropF V))) l,
...
Lemma In_InT_seqs : forall (seq : rel (list (MPropF V))) seqs, In seq seqs -> InT seq seqs.
Proof.
...
Lemma list_split_form : forall (A B : MPropF V) l0 l1 l2 l3, (l0 ++ A :: l1 = l2 ++ B :: l3) ->
                          ((A = B) * (l0 = l2) * (l1 = l3)) +
