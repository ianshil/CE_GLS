...
(* In this file we show that the applicability of the rules in PSGLS is decidable. *)
...
Lemma remove_add_rest_gen_ext : forall l (A : MPropF V), rest_gen_ext [A] (remove eq_dec_form A l) l.
Proof.
induction l.
...
Lemma in_atom_gen_ext_atom_or_in_list : forall (l1 l2 : list (MPropF V)), atom_gen_ext l1 l2 -> (forall A, In A l2 ->
                    ((existsT2 P, A = # P) + (A = Bot V) + (In A l1))).
Proof.
...
Lemma dec_is_atomicT : forall (A : MPropF V), (is_atomicT A) + ((is_atomicT A) -> False).
Proof.
induction A.
- left. left. exists w. reflexivity.
...
Lemma dec_is_boxedT : forall (A : MPropF V), (is_boxedT A) + ((is_boxedT A) -> False).
Proof.
induction A.
...
Lemma dec_is_box : forall (A : MPropF V), (existsT2 B, A = Box B) + ((existsT2 B, A = Box B ) -> False).
Proof.
destruct A.
...
Lemma dec_box_in : forall s, (existsT2 (A : MPropF V), (In (Box A) (fst s)) /\ (In (Box A) (snd s))) +
                             ((existsT2 (A : MPropF V), (In (Box A) (fst s)) /\ (In (Box A) (snd s))) -> False).
Proof.
intro. destruct s.
induction l.
- right. intro. destruct H. destruct a. simpl in H. inversion H.
...
Lemma dec_GLS_init_rules : forall (s : rel (list (MPropF V))) ,
...
Lemma dec_IdP_rule : forall (s : rel (list (MPropF V))) ,
          (IdPRule [] s) +
          ((IdPRule [] s) -> False).
Proof.
intro s. destruct s. destruct (dec_prop_var_in (pair l l0)).
...
Lemma dec_IdB_rule : forall (s : rel (list (MPropF V))) ,
          (IdBRule [] s) +
          ((IdBRule [] s) -> False).
Proof.
...
Lemma dec_BotL_rule : forall (s : rel (list (MPropF V))) ,
          (BotLRule [] s) +
          ((BotLRule [] s) -> False).
Proof.
intro s. destruct s. destruct (In_dec l (Bot V)).
...
Lemma dec_init_rules : forall (s : rel (list (MPropF V))) ,
...
Lemma dec_is_imp : forall (A : MPropF V), (existsT2 B C, A = Imp B C) + ((existsT2 B C, A = Imp B C) -> False).
Proof.
destruct A.
...
Lemma dec_imp_in : forall (l : list (MPropF V)), (existsT2 A B, In (Imp A B) l) + ((existsT2 A B, In (Imp A B) l) -> False).
Proof.
induction l.
...
Lemma dec_ImpR_rule : forall (concl : rel (list (MPropF V))),
...
- right. intro. destruct X. inversion i. subst. apply f. exists A. exists B. apply in_or_app.
  right. apply in_eq.
Qed.

Lemma dec_ImpL_rule : forall (concl : rel (list (MPropF V))),
...
- right. intro. destruct X. destruct s. inversion i. subst. apply f. exists A. exists B.
  apply in_or_app. right. apply in_eq.
Qed.

Lemma dec_box_in_list : forall l, (existsT2 (A : MPropF V), In (Box A) l) +
                             ((existsT2 (A : MPropF V), In (Box A) l) -> False).
Proof.
induction l.
...
Lemma dec_GLR_rule : forall (concl : rel (list (MPropF V))),
...
Lemma dec_GLS_rules : forall (concl : rel (list (MPropF V))) ,
...
Lemma dec_PSGLS_rules : forall (concl : rel (list (MPropF V))) ,
