...
(* To use the next fixpoint definition we need to use the definition remove.
   As this function requires the decidability of the equality on the base type,
   then we require the following hypothesis. Intuitively it seems harmless and
   in fact pretty close to reality. *)
...
Lemma NoDup_incl_lengthT : forall (l l' : list (MPropF V)), NoDup l -> incl l l' ->
...
Fixpoint n_imp_subformF (A : MPropF V) : nat :=
match A with
 | # P => 0
 | Bot _ => 0
...
Fixpoint n_imp_subformLF (l : list (MPropF V)) : nat :=
match l with
  | [] => 0
...
Definition n_imp_subformS (s : rel (list (MPropF V))) : nat :=
...
Lemma In_n_imp_subformLF_is_non_0 (l : list (MPropF V)) :
...
Theorem n_imp_subformS_is_0 (s : rel (list (MPropF V))) :
...
Fixpoint subform_boxesF (A : MPropF V) : list (MPropF V) :=
match A with
 | # P => nil
 | Bot _ => nil
...
Fixpoint subform_boxesLF (l : list (MPropF V)) : list (MPropF V) :=
match l with
  | nil => nil
  | h :: t => (subform_boxesF h) ++ (remove_list (subform_boxesF h) (subform_boxesLF t))
end.

Definition subform_boxesS (s : rel (list (MPropF V))) : list (MPropF V) :=
  (subform_boxesLF (fst s)) ++ (remove_list (subform_boxesLF (fst s)) (subform_boxesLF (snd s))).

Lemma In_subform_boxesF_box : forall (A B : MPropF V), In A (subform_boxesF B) -> is_boxedT A.
Proof.
...
Lemma In_subform_boxesLF_box : forall (A : MPropF V) l, In A (subform_boxesLF l) -> is_boxedT A.
Proof.
...
(* Then next two lemmas should be moved to a more general place. *)

Fixpoint length_form (a: MPropF V) : nat :=
match a with
  | # v => 1
  | Bot _ => 1
...
Lemma in_subform_boxesF_smaller_length_form : forall (A B : MPropF V), In A (subform_boxesF B) ->
...
Lemma RHS_top_box_is_subformLF : forall (s : rel (list (MPropF V))) (A : MPropF V),
...
Definition usable_boxes (s : rel (list (MPropF V))) : list (MPropF V) :=
...
Lemma no_RHS_box_no_GLR (s : rel (list (MPropF V))) :
...
Lemma all_RHS_boxes_are_LHS_boxes_no_GLR (s : rel (list (MPropF V))) :
...
Lemma no_usable_boxes_all_RHS_are_LHS (s : rel (list (MPropF V))) :
...
Theorem length_usable_boxes_is_0 (s : rel (list (MPropF V))) :
...
Definition term_meas (s : rel (list (MPropF V))) : nat * nat :=
  pair (length (usable_boxes s)) (n_imp_subformS s).

(* I do not necessarily need to define this pair: doing an induction first on
   length (usable_boxes s) and then a subinduction on n_imp_subformS s would
   do. I am not sure which benefit we get from using this pair. *)
...
(* Third, to prove that proof search terminates we need to show that if an initial rule 
   is applicable to a sequent then its derivations have a height less or equal to 1. *)

(* Theorem init_s_derrec_height_le_1 : forall s, ((IdPRule [] s) + (IdBRule [] s) + (BotLRule [] s)) ->
                                 forall (D : derrec (@PSGLS_rules V) (fun _ => True) s), derrec_height D <= 1.
Proof.
intros s RAInit. destruct RAInit. destruct s0.
- intro D. induction D ; inversion i.
  * simpl. auto.
  * simpl. inversion r ; try inversion H ; subst ; try pose (dersrec_height_nil d) ;
    try assert (E: dersrec_height d = 0) ; try apply e ; try reflexivity ; try rewrite E ; try auto ;
    try exfalso ; apply H ; assumption.
- intro D. induction D ; inversion i.
  * simpl. auto.
  * simpl. inversion r ; try inversion H ; subst ; try pose (dersrec_height_nil d) ;
    try assert (E: dersrec_height d = 0) ; try apply e ; try reflexivity ; try rewrite E ; try auto ;
    try exfalso ; try apply H1 ; assumption.
- intro D. induction D ; inversion b.
  * simpl. auto.
  * simpl. inversion r ; try inversion H ; subst ; try pose (dersrec_height_nil d) ;
    try assert (E: dersrec_height d = 0) ; try apply e ; try reflexivity ; try rewrite E ; try auto ;
    try exfalso ; try apply H2 ; assumption.
Qed. *)
...
(* Fourth, we need to prove that if no PSGLS rule is applicable to a sequent s,
   then its derivations have a height equal to 0. *)

Theorem no_PSGLS_rule_applic : forall s, (forall ps, (PSGLS_rules ps s) -> False) ->
                                 forall (D : derrec (@PSGLS_rules V) (fun _ => True) s), derrec_height D = 0.
Proof.
...
(* Fifth, we show that the number of usable boxes decreases upwards in the application
...
(* Theorem PSGLS_rules_no_init_measure_decreases : forall (s : rel (list (MPropF V))),
              (((IdPRule [] s) + (IdBRule [] s) + (BotLRule [] s)) -> False) ->
              (forall prems prem, (In prem prems) -> (PSGLS_rules prems s) ->
                ((length (usable_boxes prem) < length (usable_boxes s)) \/
                (length (usable_boxes prem) <= length(usable_boxes s) /\
                   n_imp_subformS (prem) < n_imp_subformS s))).
Proof.
intros. inversion X.
- exfalso. inversion H1. subst. apply H. auto.
- exfalso. inversion H1. subst. apply H. auto.
- exfalso. inversion H1. subst. apply H. auto.
- subst. right. inversion H1. subst. assert (prem = (Γ0 ++ A :: Γ1, Δ0 ++ B :: Δ1)).
  inversion H0. auto. inversion H2. rewrite H2.
  pose (@ImpR_applic_less_Imp_same_usable_boxes (Γ0 ++ A :: Γ1, Δ0 ++ B :: Δ1) (Γ0 ++ Γ1, Δ0 ++ A → B :: Δ1) H1).
  destruct p. destruct s.
  * split. lia. assumption.
  * split. lia. assumption.
- subst. right. inversion H1. subst. pose (ImpL_applic_less_Imp_same_usable_boxes H1). destruct p.
  destruct p. destruct p0. inversion H0.
  * subst. destruct s.
    + split. lia. assumption.
    + split. lia. assumption.
  * inversion H2. subst. destruct s0.
    + split. lia. assumption.
    + split. lia. assumption.
    + inversion H3.
- subst. left. inversion X0. subst. apply GLR_applic_less_usable_boxes in X0.
  inversion H0. subst. assumption. inversion H5. intro. apply H. auto.
Qed. *)
